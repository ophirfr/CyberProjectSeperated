    ...
    def LogIn(self, api_url, password, userName):
        ...
        try:
            response = requests.post(URL, data={}, auth=(userName, password),
                                     verify=False)  # do not check certificate (חתימה עצמאית)
            return response, response.status_code
        except (requests.ConnectionError, requests.Timeout):
            return "No internet connection.", 500
...
def main():
    ...
    res, status = rest.CompareImages("http://127.0.0.1:5000", 1,
                                     r"C:\Users\ophir\Desktop\FinalProject\test_jpg\ophir.jpeg")
